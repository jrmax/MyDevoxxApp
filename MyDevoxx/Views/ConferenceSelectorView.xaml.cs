    ...
    public sealed partial class ConferenceSelectorView : Page
    {
        ...
        private void GoButton_Click(object sender, Windows.UI.Xaml.RoutedEventArgs e)
        {
            if (vm.Conference != null && vm.Conference.id != null && !isLoading)
            {
                ...
                settings.Values[Settings.CONFERENCE_ID] = vm.Conference.id;
                settings.Values[Settings.CFP_ENDPOINT] = vm.Conference.cfpEndpoint;
                settings.Values[Settings.TALK_URL] = vm.Conference.talkURL;
                settings.Values[Settings.COUNTRY] = vm.Conference.country;
                settings.Values[Settings.VOTING_URL] = vm.Conference.votingURL;
                settings.Values[Settings.PUSH_UPDATE_ENABLED] = vm.Conference.pushEnabled;
                settings.Values[Settings.LOADED_SUCCESSFUL] = false;
        ...
        private void finishedLoading(bool successful)
        {
            ...
            if (successful)
            {
                settings.Values[Settings.LOADED_SUCCESSFUL] = true;

                var notificationService = ServiceLocator.Current.GetInstance<INotificationService>();
                notificationService.TryRegisterAndSubscribeKnownTopics();
